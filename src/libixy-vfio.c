...
int vfio_init(const char* pci_addr) {
	...
	check_err(((group_status.flags & VFIO_GROUP_FLAGS_VIABLE) > 0) - 1,
...
uint8_t* vfio_map_region(int vfio_fd, int region_index) {
	...
	return (uint8_t*) check_err(mmap(NULL, region_info.size, PROT_READ | PROT_WRITE, MAP_SHARED, vfio_fd, region_info.offset),
								"mmap vfio bar0 resource");
}
