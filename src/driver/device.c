...
struct ixy_device* ixy_init(const char* pci_addr, uint16_t rx_queues, uint16_t tx_queues, int interrupt_timeout, bool interrupts_enabled) {
	...
	if (vendor_id == 0x1af4 && device_id >= 0x1000) {
		...
		return ixgbe_init(pci_addr, rx_queues, tx_queues, interrupt_timeout, interrupts_enabled);
	}
}
